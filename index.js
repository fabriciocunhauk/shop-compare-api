...
import { exec } from 'child_process'; // Import exec for running Ollama CLI command
...
async function extractTextFromImage(imagePath) {
 const worker = await createWorker('eng');
  ...
  const { data: { text } } = await worker.recognize(imagePath); // Perform OCR
...
async function parseTextWithOllama(text) {
  const prompt = `Extract the product names, prices, and supermarket name from the following receipt text:\n${text}`;
